...
String convert_unary(void)
{
  ...
  if (isalnum(*inp))
  {
    ...
    if (atom.s_length == 7 && memcmp("bzwsize", atom.s_string, 7) == 0)
    {
      atom = (String) { "8", 1 };
    }
    else if (is_integer)
...
int main(int argc, char *argv[])
{
  ...
  fprintf(output_stream, "%s\n%s\n%s\n%s\n%s\n",
    ...
    "  return ((uint8_t *)s)[i];\n"
    "}\n\n"
    "W "NAME_PREFIX"close(W fd)\n" "{\n"
    "  return close(fd);\n"
    "}\n\n"
    "W "NAME_PREFIX"open(W path, W writing)\n" "{\n"
    "  W fd;\n\n"
    "  fd = open((const char *)path, writing ? O_CREAT | O_RDWR | O_TRUNC : O_RDONLY, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);"
    "  _setmode(fd, _O_BINARY);\n"
    "  return fd;\n"
    "}\n\n"
    "W "NAME_PREFIX"creat(W path, W mode)\n" "{\n"
    "  return "NAME_PREFIX"open(path, 1);\n"
    "}\n\n"
    ...
    "}\n\n"
    "W "NAME_PREFIX"seek(W fd, W offset, W type)\n" "{\n"
    "  return lseek(fd, offset, type == 0 ? SEEK_SET : type == 1 ? SEEK_CUR : SEEK_END);\n"
    "}\n\n"
    "W "NAME_PREFIX"write(W fd, W buff, W size)\n" "{\n"
    "  return write(fd, (void *)buff, size);\n"
    "}\n\n"
    "W "NAME_PREFIX"bzg8(W ptr)  { return *(const uint8_t  *)ptr; }\n"
    "W "NAME_PREFIX"bzg16(W ptr) { return *(const uint16_t *)ptr; }\n"
    "W "NAME_PREFIX"bzg32(W ptr) { return *(const uint32_t *)ptr; }\n"
    "W "NAME_PREFIX"bzg64(W ptr) { return *(const int64_t  *)ptr; }\n"
    "W "NAME_PREFIX"bzs8(W ptr, W value)  { *(uint8_t  *)ptr = value; return ptr; }\n"
    "W "NAME_PREFIX"bzs16(W ptr, W value) { *(uint16_t *)ptr = value; return ptr; }\n"
    "W "NAME_PREFIX"bzs32(W ptr, W value) { *(uint32_t *)ptr = value; return ptr; }\n"
    "W "NAME_PREFIX"bzs64(W ptr, W value) { *(int64_t  *)ptr = value; return ptr; }\n"
    "W "NAME_PREFIX"bzmalloc(W n)                             \n"
    "{                                           \n"
    "  char *p;                                  \n"
    "                                            \n"
    "  n += 8;                                   \n"
    ...
    "  return (W)p + 8;                          \n"
    "}                                           \n"
    "W "NAME_PREFIX"bzrelloc(W oldptr, W newsize)             \n"
    "{                                           \n"
    "  char *p;                                  \n"
    "  W oldsize;                                \n"
    "                                            \n"
    "  if (oldptr == 0) return "NAME_PREFIX"bzmalloc(newsize);\n"
    "  newsize += 8;                             \n"
    "  oldsize = *(W*)(oldptr - 8);              \n"
    "  if (newsize <= oldsize)                   \n"
    "  {                                         \n"
    ...
    "    return oldptr;                          \n"
    "  }                                         \n"
    "  p = realloc((char *)oldptr - 8, newsize); \n"
    "  if (p == NULL) return 0;                  \n"
